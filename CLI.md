...
`vt`는 터미널 어디서든 실행 가능한 단일 진입점입니다. 이 문서는 전체 명령/옵션의
상세 레퍼런스입니다 — 개요는 [README.md](./README.md), 개념 설명은 `vt help concepts`를
참고하세요.
...
| 명령 | 설명 |
|------|------|
| `vt voice` | 음성 모드 — 서버 + Voice Daemon 백그라운드 시작 (노션 등 다른 작업 중에도 사용 가능) |
| `vt clip` | 클립보드 동기화 데몬 — 맥 클립보드 변경을 웹으로 push (OSC52로 못 잡는 터미널 밖 복사 보완) |
| `vt mobile [옵션]` | 모바일 접속 URL + QR 코드 출력 |
| `vt start` | 전체 시작 — 서버 + 터널 + 음성 데몬 |
| `vt stop [--purge]` | 종료 — `--purge`는 tmux 세션까지 완전 종료 |
| `vt status` | 서버·터널·Voice Daemon·tmux 상태 확인 |
| `vt manage` | TUI 관리 도구 — 세션 목록/rename/kill/attach + 핫키/상태 조회 |
| `vt attach [name]` | 임의 tmux 세션을 새 OS 터미널 창에 attach |
| `vt voice-target [name\|--auto]` | Voice Daemon 타깃 세션 lock/해제 |
| `vt queue [하위명령]` | 프롬프트 큐 — 작업 중 지시를 쌓아뒀다 순차 투입 (아래 [프롬프트 큐](#프롬프트-큐-vt-queue) 참고) |
| `vt hotkey [list\|set\|reset\|disable]` | 핫키 조회/변경 |
| `vt password [clear]` | 웹 로그인 비밀번호 설정(해시만 저장) / `clear`로 해제 |
| `vt otp [status\|setup\|disable]` | 새 기기 등록 시 OTP 요구 — `setup` 전까지 완전 비활성 |
| `vt device [list\|revoke <id>]` | 등록된 기기 조회 / 폐기 (폰 분실 시 세션까지 함께 무효화) |
| `vt help <topic>` | 토픽별 도움말 — `concepts`/`voice`/`hotkeys`/`target`/`troubleshoot`/`webui`/`ssh`/`tunnel-hook` |
...
| `vt run "..."` | headless `claude -p` 백그라운드 실행 + TTS 알림 |
| `vt tunnel expose <port> "이름"` | 다른 로컬 포트를 별도 Cloudflare 터널로 공개 |
| `vt tunnel unexpose <port>` | 해당 포트 터널 종료 |
| `vt tunnel list` | 열려 있는 터널 전부 (메인 + 추가 포트) |
| `vt tunnel hook` | URL 변경 훅 확인 + 즉시 실행 (자세히: `vt help tunnel-hook`) |
| `vt tunnel restart` | 좀비 재연결(응답 없음) 상태여도 강제로 새 터널 기동 + 훅 재실행 |
| `vt tunnel watchdog` | 좀비 재연결 자동 감지 데몬 상태 확인/시작 (평소엔 자동 기동) |
| `vt ssh [session]` | Tailscale + SSH로 tmux 세션 직접 접속 — 회사망 등 화면 원격이 막힌 환경 (자세히: [아래](#tailscale--ssh-원격-접속)) |
| `vt doctor` | 설치/환경 진단 — 아래 [점검 항목](#vt-doctor-점검-항목) 참고 |
| `vt install-profiles [--dry-run]` | 터미널 앱 profile 자동 등록 (iTerm2 Dynamic Profile + 기타 snippet) |
| `vt shell-init [zsh\|bash\|fish\|pwsh]` | 셸별 안전 통합 스니펫 출력 (`eval "$(vt shell-init zsh)" >> ~/.zshrc`) |
...
vt mobile --safe                      # 위험 명령(rm -rf /, sudo 등) 사전 차단
vt mobile --network <mode>            # localhost | lan | tailscale | all (기본값)
vt mobile --force                     # 인증(비밀번호/토큰) 미설정이어도 공개 터널 강행 — 비권장
```

`--network all`(기본값)로 공개 터널을 열려면 `vt password` 또는 `VT_AUTH_TOKEN`으로
...
```bash
vt queue list                  # 큐 목록
vt queue add "다음 지시" [세션]  # 큐에 추가 (상한 50개)
vt queue run                   # 수동 드레인 — 한 건 투입
vt queue rm <id>                # 항목 삭제 (id=all이면 전체 비우기)
vt queue unblock <id>          # safe_mode에 막힌 항목 재개
vt queue clear                 # 전체 비우기
```

자동 투입은 **Claude Code의 Stop 훅에서만** 걸립니다. codex/aider/gemini는 훅이
없어 `vt queue run` 또는 웹 UI의 "지금 실행"으로 수동 투입해야 합니다. 투입 전
...
vt ssh --add-key "ssh-ed25519 AAAA... user@laptop"   # 공개키 등록
```
...
| `vt mobile` (기본, `--network all`) | 인증(비밀번호/토큰) | Tailscale 없는 완전 외부 기기 |
...
`vt` CLI · server · Voice Daemon · hook이 모두 `-L vt` 격리 소켓을 사용 → 모든
