...
- `VT_AUTH_TOKEN`은 데몬/QR용 **기계 토큰**으로 그대로 병존한다. 사람은 비밀번호로,
...
def make_session(ttl: int = SESSION_TTL) -> str:
    ...
    """
    if VT_AUTH_SESSION_KEY:
        exp = int(time.time()) + ttl
        payload = f"v1.{exp}"
        sig = hmac.new(VT_AUTH_SESSION_KEY.encode("utf-8"), payload.encode("utf-8"), hashlib.sha256).hexdigest()
        return f"{payload}.{sig}"
    return VT_AUTH_TOKEN  # 레거시: 서명키 미설정 시 기존처럼 토큰을 쿠키로
...
def verify_session(value: str) -> bool:
    ...
    try:
        ...
        payload = f"{ver}.{exp_s}"
        expected = hmac.new(VT_AUTH_SESSION_KEY.encode("utf-8"), payload.encode("utf-8"), hashlib.sha256).hexdigest()
