...
Voice Daemon(`fsh voice` 데스크톱 핫키)이 어느 tmux 세션으로 입력을 보낼지 제어합니다.
...
## 명령
    ...
    fsh voice-target              # 현재 모드 + 사용 가능한 세션 목록
    fsh voice-target dev          # dev 세션으로 LOCK
    fsh voice-target --auto       # LOCK 해제, AUTO로 복귀
...
### 시나리오 A: 노션 작업 중 음성으로 코딩
    fsh voice                # daemon 시작
    fsh voice-target dev     # dev에 lock
    [노션에서 작업]
