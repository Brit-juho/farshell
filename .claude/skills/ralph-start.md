...
```bash
# 이미 실행 중인 서버 확인
lsof -i :8000 -t 2>/dev/null && echo "SERVER_ALREADY_RUNNING" || echo "SERVER_NOT_RUNNING"
```
...
nohup /opt/homebrew/Caskroom/miniforge/base/envs/whisper/bin/python -m uvicorn main:app --host 0.0.0.0 --port 8000 > /tmp/ralphton-server.log 2>&1 &
echo "SERVER_PID: $!"
```

3초 후 헬스체크:
```bash
curl -sf http://localhost:8000/ -o /dev/null && echo "SERVER_OK" || echo "SERVER_FAIL"
```
...
| 같은 네트워크 | http://$(ipconfig getifaddr en0):8000 |
| 원격 (어디서든) | {터널 URL} |
