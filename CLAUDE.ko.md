...
| 파일 업로드 | 키바 📎 슬롯(모바일 — 키바를 접어도 안 사라진다) 또는 `Mod+K` → "파일 업로드", **터미널에 이미지를 붙여넣어도 업로드된다**. `/tmp/vt-uploads/`에 저장(디렉토리 0700, 파일 0600)하고 결과 경로를 pane에 자동으로 타이핑한다. 용량 상한 `VT_MAX_UPLOAD_MB`(기본 200)를 스트리밍으로 검사해 초과 시 413. `#file-input` 엘리먼트를 세 진입점이 공유한다 — 두 번째 input을 만들지 말 것 |
| 파일 다운로드 | `GET /api/download?path=...` — **API 전용이고 UI가 없다**(`frontend/`에 참조 0건). `/tmp/vt-uploads` 밖은 403이라 "올린 걸 되받는" 용도지 맥의 임의 파일을 가져오는 기능이 아니다. 파일을 빼내려면 여전히 수동으로 `cp <경로> /tmp/vt-uploads/` 후 URL을 건네야 한다 |
