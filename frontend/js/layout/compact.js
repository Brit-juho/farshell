...
// 활성 leaf 하나만 화면 전체로 그리고, 나머지는 표면 레이어(layout/surface.js)가
// 화면 밖으로 숨긴다(렌더링은 layout/panes.js가 한다 — 이 파일은 "지금이
// compact인가"와 "leaf를 어떤 순서로 나열할까"만 안다). 트리 자체는 건드리지
// 않는다 — 착수 전 설계 리뷰 원칙 1 그대로: compact는 "다르게 그리는 렌더
// 모드"이지 "다른 트리"가 아니다.
//
