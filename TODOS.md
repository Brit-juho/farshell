...
- [ ] **[D16] 코드 뷰어에 Git 액션(stage/commit) 추가**
  - **What:** 지금은 `/api/git/status`·`/api/git/diff`로 완전 읽기 전용인 코드 뷰어에, 딱
    stage/commit 정도의 최소 쓰기 액션을 추가. push나 브랜치 조작 같은 더 위험한 동작은
    범위 밖
  - **Why:** 경쟁 서비스(9Remote 등) 대비 격차로 확인됨 — 걔넨 코드 에디터에 Git 연동이
    이미 있음. 지금은 diff를 보고도 커밋하려면 결국 터미널로 돌아가야 해서 "보기 전용"의
    가치가 반쪽
  - **Context:** 2026-08-19 경쟁 프로젝트 비교 분석(9Remote/Remux 등, `mobile-terminal`/
    `remote-terminal` GitHub 토픽 조사)에서 발견. `server/routes/files.py`,
    `server/fsguard.py` 참고
  - **주의:** 이번 리뷰에서 코드 뷰어의 핵심 방어 전제가 "읽기 전용이라 쓰기 API가 아예
    없다"는 것이었음(공개 터널로 노출되는 걸 감안한 설계). 쓰기 액션을 추가하는 순간 그
    전제가 깨지므로, 인증·CSRF·경로 검증(`fsguard`)을 stage/commit 엔드포인트에도 동일하게
    적용하고 스코프를 최소(딱 stage/commit)로 좁게 유지할 것 — push/브랜치 조작으로 확장
    금지
  - **Trigger:** 사용자가 방향 재확인한 뒤 착수
