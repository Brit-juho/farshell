...
| Header tabs = worktrees (10 §4, stage 2) | The header now has a **worktree tab strip** (`#vt-wtabs`) and **each tab owns its own pane tree** — switch away and back and that worktree's split is exactly as you left it. The layout snapshot moved to `v2` (`{tabs:[{id,worktreeId,label,tree}], activeTab}`); a `v1` snapshot means "one tab", and the v1 fields are still written **for rollback** (roll 2.1.2 back and the old code still finds a layout). The per-session tabs (`#tabs`) stay — 26 places read `session.tabEl` for names and agent badges — but are filtered to the active worktree, so they read as the session row inside that tab. A tab hides itself when there is only one, closing a tab never kills a session, and the last tab can't be closed (no tree left to render). Same session is never placed in two tabs: the surface layer moves one DOM node around. |
