...
- **설정 화면의 정보 구조와 모바일 탐색을 다시 구성했다.** 데스크톱은 880px
  중앙 모달 안에서 7개 범주와 관련 항목 그룹을 한눈에 찾게 했고, 모바일은
  마지막 탭을 `더보기`로 바꿔 파일·포트·사용량 도구와 설정 목록을 먼저 보여준
  뒤 상세로 이동하게 했다. 각 설정에는 적용 범위(`이 기기`/`모든 기기`)와
  자동 저장 상태를 표시하고, 폼 라벨 연결·44px 터치 영역·슬라이더 숫자 입력을
  추가했다. 설정 검색, 변경된 값을 기본값으로 되돌리는 행 액션, 스크롤백의
  2천·5천·1만 줄 빠른 선택도 제공한다.
...
- **서버가 기동조차 되지 않았다 — 없는 심볼을 import 하고 있었다.**
  ...
  엔드포인트 5개(`/api/codex/managed/*`)가 적혀 있었을 뿐 구현이 없었다 —
  문서·import·레일 표시 경로(관리형 배지, `phase` 문장, `AgentDetail.phase/
  source`)를 전부 걷어냈다. 레일 쪽은 공급하는 쪽이 없어 항상 `null`이었다.

- **dock이 좁아지면 설정·마이크·접기 버튼이 통째로 사라졌다.**
  `.vt-dock-tabs`가 `overflow:hidden`이라, 최소 폭(320px) 근처에서 탭 5개 +
  버튼 3개의 합계가 넘치면 뒤쪽이 잘려 **보이지도 눌리지도** 않았다 — dock을
  좁게 쓰면 접기 버튼 자체를 잃는다. 탭 목록만 자체 스크롤로 감싸고(헤더
  `#vt-wtabs`와 같은 관용구) 액션 버튼은 그 밖에 뒀다. 곁들여: 펼친 dock의
  배지를 점에서 **수치**로 되돌렸고(점으로 내린 이유는 접힘 36px의 폭 초과인데
  펼침엔 해당이 없고, hover가 없는 터치 기기는 점 뒤의 수를 영영 못 봤다),
  줄 높이를 30 → 28px로(DESIGN.md §10의 24/28/36 세 단), 마이크를 설정 앞으로
  (접힘은 이미 그 순서였는데 펼침만 반대였다), 접기 화살표를 손으로 쓴 SVG에서
  `icons.js` 호출로 바꾸고 실제 접히는 방향(오른쪽)을 가리키게 돌렸다.

- **소스컨트롤 dock에서 저장소 이름이 "far…"로 잘렸다.** `.vt-scm-repo`/
  `.vt-scm-branch`에 `min-width`가 없었다 — 부모가 `overflow:hidden`이면
  flex item의 자동 최소폭이 0으로 떨어지므로(스펙), 같은 줄의 `flex:none`
  배지들(브랜치·업스트림·↑↓·±·파일 수)이 폭을 그대로 지키는 동안 이름만
  밀렸다. dock 기본폭 392px에서도 잘렸다. 주석이 "이름이 주인공"이라고 적어둔
  것을 실제로 지키려면 최소폭이 필요하다.

- **음성 모듈을 못 불러와도 화면은 아무 말이 없었다.** capability는 `true`인데
  산출물(`/static/dist/voice.js`)이 없거나 깨진 경우(빌드 누락 — 2026-09-18
  실사고) 콘솔 404만 남고 `.needs-voice` 버튼은 눌리는 채로 방치됐다. 사용자
  눈에는 "눌러도 반응 없음"으로만 보인다. `script.onerror`에서 토스트를 띄우고
  그 버튼들을 비활성으로 돌린다.

- **큐·사용량 dock 패널의 빈 상태가 위에 붙어 아래가 텅 비어 보였다.** 빈 상태
  문장 하나뿐인 `.vt-vw-body`만 `:has()`로 골라 세로 중앙 정렬한다 — 실제 목록이
  있는 body(코드 뷰어·파일 트리)는 이 선택자에 안 걸려 그대로 위에서부터 쌓인다.
...
- **받아둔 Whisper 모델을 설정에서 지운다.** 설정 → 「음성」에 「다운로드된 모델」
  행이 생겼다. 위의 「Whisper 모델」 행이 메모리 적재/해제(가볍다)라면 이쪽은
  디스크에 받아둔 가중치(수백MB) 자체를 지워 공간을 돌려받는 쪽이다 — 성격이
  달라 별개 행으로 둔다. `GET`/`DELETE /api/voice/stt/model`. 두 가지를 의도적으로
  좁혔다: HF 캐시 **전체**가 아니라 이름에 `whisper`가 든 리포만 대상으로 한다
  (같은 캐시를 사용자가 따로 받은 다른 로컬 모델과 공유할 수 있고, 그걸 지우는 건
  이 기능의 책임 밖이다), 그리고 `DELETE`는 받은 경로를 믿지 않고 직전 `GET`
  목록에 있는 것만 지운다(경로 조작으로 캐시 밖 파일을 지우는 것 차단). 지우기
  전에 먼저 언로드한다.

- **tmux 세션 이름의 "tmux:" 접두사가 실사용 화면에 계속 남아 있던 문제
  (사용자 피드백, 2026-09-19).** 서버(`routes/tmux.py`)의 접두사 생성은
  이미 지웠는데도, **예전에 생성된 세션의 `displayName`에 그 접두사
  문자열이 그대로 저장돼 있어서**(워크스페이스 스냅샷에 박혀 서버를
  고쳐도, 재시작해도 안 지워진다) "tmux:1", "tmux:2" 같은 이름이 계속
  보였다. `core/store.js`의 `sessionDisplayName()`이 이제 `displayName`이
  정확히 `tmux:<그 세션의 tmux 이름>` 꼴이면(사용자가 진짜 지은 이름일
  리 없는 옛 기본값) 무시하고 깨끗한 tmux 이름을 대신 보여준다. 사용자는
  또한 애초에 "이름 앞에 문자열을 붙여 구분하는" 방식 자체를 원치
  않았다("이름에 붙는 게 아니라 다른 방식으로 구분되게") — tmux 세션
  여부를 이제 이름 옆의 작은 아이콘 배지(`RailRow.tsx`, `plug` 아이콘,
  일반 세션엔 아예 안 그린다)로 표시한다.
...
- **포트 행의 「외부」 태그를 없앴다.** 모든 인터페이스에 열린 포트(`*`)에 경고색
  pill을 붙이고 있었는데, 개발 중 서버는 대부분 그 상태라 거의 모든 행이 상시
  경고처럼 보였다 — 경고가 기본값이면 경고가 아니다. 같은 정보는 행의 부제가
  이미 말한다.

- **파일 행의 삭제 아이콘을 hover 전까지 숨긴다.** 행마다 아이콘 5개(다운로드·
  경로 복사·삽입·링크·삭제)가 같은 무게로 늘어서 있어 파괴적 동작이 구분되지
  않았다. 포트 패널의 종료 버튼과 같은 관용구다 — hover가 없는 터치 기기는
  영영 못 찾으므로 그대로 상시 노출한다.
