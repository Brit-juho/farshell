...
| `--color-acc-surface` | `var(--color-acc)` | 액센트 위에 `acc-ink` 텍스트가 얹히는 면(주 액션 버튼·배지·마이크 버튼). 기본은 액센트 그대로고 **macos만 `acc-dim`으로 재정의** — 애플 시스템 블루 위 흰 글자가 3.65:1이었다 |
...
2026-09-08까지 **28조합이 미달**이었다. farshell만 처음부터 대비를 계산해 만들었고,
나머지 5스킨은 각 OS의 색을 그대로 옮겨왔기 때문이다 — macos `sub`는 bg-3에서
3.49:1, vscode `sub`는 bg-1에서 3.73:1이라 야외나 밝은 화면에서 실제로 안 읽혔다.

고칠 때 **색상(hue)과 채도를 그대로 두고 명도만 최소한으로 움직였다.** 그 스킨이
해당 OS를 흉내내는 게 정체성이라, 색조까지 바꾸면 흉내가 아니게 된다. 실제로 바뀐
건 7개 값이고 H·S는 소수점까지 동일하다(예: macos `sub` `#8e8e96`→`#a3a3a9`,
H 240° S 4% 유지). `st-idle`은 6스킨 전부 `muted`와 같은 값이라 함께 움직인다.
