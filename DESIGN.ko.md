...
- 버튼 뒤에 같은 색을 흐린 **글로우 그림자를 두지 않는다.** 그림자는 회색·저투명도로,
  높이(elevation)를 말하는 **두 자리에만** 쓴다: 오버레이(팔레트·시트·다이얼로그)는
  `--shadow-e2/e3`, **지금 보고 있는 행 하나**는 `--shadow-e1`. 그 외의 경계는
  1px 선(`--color-line`)으로 표현한다.

  `--shadow-e1`은 2.1.0에서 지웠다가 **2.1.6에서 되살렸다.** 이유를 남겨 둔다:
  지운 뒤에도 `.vt-wgrail-row.active`가 계속 이 변수를 쓰고 있었는데 정의는
  `notepad` 스킨 안에만 있었다. 정의 안 된 변수를 쓴 `box-shadow` 선언은 통째로
  무효가 되므로, 나머지 5스킨에서 활성 행은 배경 한 단차만 남았다 — farshell에서
  레일 대비 1.06:1로 사실상 같은 색이었다. "지금 어느 행에 있나"를 이름의 액센트
  글자색 하나가 떠받치고 있었다는 뜻이다. e1은 e2보다 훨씬 약하게 잡는다:
  e1은 "목록 안에서 한 행이 들렸다", e2는 "패널이 화면 위에 떠 있다".
...
- **반경:** `--radius-sm`(**5px**, 모든 컨트롤) / `--radius-lg`(**10px**, 바깥 프레임·
  ...
  스킨 안에서 덮어쓴다.
- **그림자:** `--shadow-e1`(지금 있는 행) + `--shadow-e2`/`e3`(오버레이).
  e1은 2.1.0에서 삭제했다가 2.1.6에서 되살렸다 — 이유는 §0 참고.
